...
const ImageGallery = ({
  ...
  useEffect(async () => {
    const photos = await getPhotos(first, assetType, after, groupTypes, groupName, mimeTypes, fromTime, toTime, include);
    setPhotos(photos.edges);
  }, []);
  return <ScrollView style={styles.container} showsVerticalScrollIndicator={false}>
      {photos.map((p, i) => {
      return <View style={styles.imageContainer} key={i}>
            <Image source={{
