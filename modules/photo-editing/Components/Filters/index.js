...
export default function Filter({
  ...
  const ItemRender = ({
    ...
    <View style={[styleSheet.item, styleSheet.AshWkxwE]}>
      <Text style={[styleSheet.itemText]}></Text>
      <Text style={[styleSheet.titleText, styleSheet.pXjPjosM]}>{name}</Text>
    </View>
    </TouchableNativeFeedback>;

  const Separator = () => {
    return <View style={styleSheet.DURuWWwV} />;
  };

  return <FlatList data={options.FILTERS} renderItem={({
    item,
    index
